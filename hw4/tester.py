...
def main():
	...
	# finds the model velocity at a given radius for some characteristic radius
	...
	for r0 in np.linspace(0.1, 10.0, 50):
		plt.subplot(211).scatter(r0, f_rss(r0))
	...
	# (a,b,c) isn't spaced optimally, the golden search will quickly converge to
	# brackets that have the correct spacing (NR pg 494).
	...
	for r0 in np.linspace(0.1, 10.0, 10):
		plt.subplot(212).plot(dat[:,0], f_model(dat[:,0],r0),'go-', alpha=0.5)

	# plot the optimal solution and observed data
	plt.subplot(212).plot(dat[:,0], f_model(dat[:,0], res[0]), 'bo-')
	plt.subplot(212).plot(dat[:,0], dat[:,1],'ro-')
